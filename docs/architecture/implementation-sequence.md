...
- TS-side hook enforcement, hook logging, Go DecisionReader, runtime hook registration và build verification path hiện đã có implementation và validation tương ứng.
...
- workspace scan, tree-sitter init, AST-first symbol extraction, import extraction, call-site extraction, chunking và index workflow runner đã có trong current implementation.
...
- embedding pipeline, persisted chunk/embedding cache, semantic search path, semantic mode wiring, re-embed flow và ANN path cho scale lớn đã có.
...
- skill/MCP registries, policy selection, audit logging và runtime hook integration path liên quan đã có theo trạng thái hoàn tất hiện tại.
...
- required-tools policy, answer gating, retry/degrade policy, audit và runtime-level enforcement path tương ứng hiện đã có trong implementation hiện tại.
...
- delivery/migration workflows, handoff chain, review/verification gates, work-item planning và dependency-aware scheduling hiện đã có theo mức hoàn tất của roadmap.
...
- `doctor` hiện đã report XDG paths, SQLite readiness, registry health, semantic config health, hook readiness, DB integrity, action guidance và machine-readable output phù hợp với trạng thái hiện tại.
