...
import (
	...
	"github.com/duypham93/dh/packages/opencode-core/internal/llm/prompt"
	"github.com/duypham93/dh/packages/opencode-core/internal/llm/provider"
...
func (a *agent) processGeneration(ctx context.Context, sessionID, content string, attachmentParts []message.ContentPart) AgentEvent {
	...
	for {
		...
		// [dh hook] Pre-Answer: check if dh allows this answer to be finalized
		preCtx := buildPreAnswerContext(content, agentMessage, toolResults)
		result, hookErr := evaluatePreAnswerPolicy(ctx, sessionID, agentMessage, preCtx)
		if hookErr != nil {
			logging.Warn("dh pre-answer hook error", "error", hookErr)
		} else if !result.allow {
			logging.Info("dh pre-answer hook rejected answer", "action", result.action)
			if result.outcome == preAnswerOutcomeRetry {
				logging.Info("dh pre-answer requested retry", "action", result.action)
				continue
			}
			return result.event
		}
...
func (a *agent) streamAndHandleEvents(ctx context.Context, sessionID string, msgHistory []message.Message) (message.Message, *message.Message, error) {
	...
	for i, toolCall := range toolCalls {
		...
		default:
			...
			// [dh hook] Pre-Tool-Exec: check if dh allows this tool call
			if allow, reason, hookErr := evaluatePreToolPolicy(ctx, sessionID, assistantMsg.ID, toolCall); hookErr != nil {
				logging.Warn("dh pre-tool-exec hook error", "tool", toolCall.Name, "error", hookErr)
			} else if !allow {
				logging.Info("dh pre-tool-exec hook blocked tool", "tool", toolCall.Name, "reason", reason)
				toolResults[i] = message.ToolResult{
					ToolCallID: toolCall.ID,
					Content:    fmt.Sprintf("Tool blocked by dh policy: %s", reason),
					IsError:    true,
				}
				continue
			}
...
func createAgentProvider(agentName config.AgentName) (provider.Provider, error) {
	...
	// [dh hook] Model Override: let dh choose a runtime model/provider override.
	if overrideProvider, overrideModel, overrideVariant, hookErr := dhhooks.OnModelOverride(string(agentName), "", ""); hookErr != nil {
		logging.Warn("dh model override hook error", "agent", agentName, "error", hookErr)
	} else {
		if overrideModel != "" {
			overriddenModel, found := models.SupportedModels[models.ModelID(overrideModel)]
			if !found {
				logging.Warn("dh model override references unsupported model", "agent", agentName, "model", overrideModel)
			} else {
				model = overriddenModel
				logging.Info("dh model override applied", "agent", agentName, "model", overrideModel, "provider", model.Provider, "variant", overrideVariant)
			}
		} else if overrideProvider != "" && models.ModelProvider(overrideProvider) != model.Provider {
			logging.Warn("dh provider override ignored without compatible model override", "agent", agentName, "requested_provider", overrideProvider, "resolved_provider", model.Provider)
		}
	}
