...
func GetAgentPrompt(agentName config.AgentName, provider models.ModelProvider) string {
	...
	if agentName == config.AgentCoder || agentName == config.AgentTask {
		// [dh hook] Skill Activation: inject active skills into the prompt
		if skills, hookErr := dhhooks.OnSkillActivation(context.Background(), "", "", "", string(agentName)); hookErr != nil {
			logging.Warn("dh skill activation hook error", "error", hookErr)
		} else if len(skills) > 0 {
			skillSection := "\n\n# Active Skills\n"
			for _, skill := range skills {
				skillSection += fmt.Sprintf("- %s\n", skill)
			}
			basePrompt = basePrompt + skillSection
			logging.Debug("dh skills injected into prompt", "skills", skills)
		}
