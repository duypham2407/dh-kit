...
func NewProvider(providerName models.ModelProvider, opts ...ProviderClientOption) (Provider, error) {
	...
	// [dh hook] Model Override: let dh override the provider/model before instantiation
	if overrideProvider, _, _, err := dhhooks.OnModelOverride(string(providerName), "", ""); err != nil {
		logging.Warn("dh model override hook error", "error", err)
	} else if overrideProvider != "" {
		logging.Debug("dh model override", "original", providerName, "override", overrideProvider)
		providerName = models.ModelProvider(overrideProvider)
	}
