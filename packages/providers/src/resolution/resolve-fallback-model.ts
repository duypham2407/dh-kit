...
export function resolveFallbackModel(agentId: string): ResolvedModelSelection {
  ...
  const provider = listProviders().find((entry) => entry.providerId === agent.defaultProvider && entry.enabled);
  if (!provider) {
    throw new Error(`Default provider '${agent.defaultProvider}' is not enabled for agent '${agentId}'.`);
  }

  const model = listModels(agent.defaultProvider).find((entry) => entry.modelId === agent.defaultModel && entry.available);
  if (!model) {
    throw new Error(`Default model '${agent.defaultModel}' is not available for agent '${agentId}'.`);
  }

  const variant = listVariants(agent.defaultProvider, agent.defaultModel).find(
